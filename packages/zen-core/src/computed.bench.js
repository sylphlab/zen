...
import { Provider, createStore as createJotaiStore, atom as jotaiAtom, useAtomValue /* useSetAtom, */, } from 'jotai'; // Removed unused useSetAtom
...
import { zen as zenCreateAtom, get as zenGetAtomValue, set as zenSetAtomValue, subscribe as zenSubscribeToAtom, } from './atom'; // Import updated functional API, alias atom as zenCreateAtom
...
const createJotaiReadBenchSetup = (atomToRead) => {
...
describe('Computed Creation', () => {
    ...
    bench('zen (1 dependency)', () => {
        ...
        zenCreateComputed([baseAtomZ], (...values) => values[0] * 2); // Use rest parameters
    });
...
describe('Computed Get (1 dependency)', () => {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: Test setup requires cast
    const computedZ = zenCreateComputed([baseAtomZ], (...values) => values[0] * 2); // Use rest parameters
    const computedN = nanoComputed(baseAtomN, (val) => val * 2);
    const computedJ = jotaiAtom((get) => get(baseAtomJ) * 2);
    const selectComputedZu = (state) => state.count * 2;
    const derivedV = {
        get computed() {
...
describe('Computed Update Propagation (1 dependency)', () => {
    ...
    const setBaseE = createEffectorEvent();

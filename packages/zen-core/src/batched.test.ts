...
describe('batched', () => {
    ...
    test('batches multiple updates within one tick', async () => {
        ...
        const derived = batched([source1 as any, source2 as any], (v1, v2) => (v1 as number) + (v2 as number)); // Cast sources, add type hints
        const listener = vi.fn();
        const unsub = subscribe(derived as any, listener); // Cast derived
    ...
    test('does not notify if calculation result reference is the same', async () => {
        ...
        const derived = batched(source as any, (value: unknown) => { // Accept unknown
            calculationCount++;

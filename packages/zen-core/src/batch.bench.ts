...
describe('Batch vs Sequential Sets (With 1 Listener Each) (functional)', () => {
   describe('2 Sets', () => {
     bench('zen batch', () => {
      const $a = zenAtom(0);
      const $b = zenAtom(0);
      zenSubscribeToAtom($a as any, () => {}); // Use functional API, add cast
      zenSubscribeToAtom($b as any, () => {}); // Add cast
      zenBatch(() => {
        zenSetAtomValue($a, 1);
        zenSetAtomValue($b, 1);
      });
   ...
   describe('5 Sets', () => {
      bench('zen batch', () => {
        ...
        atoms.forEach($a => zenSubscribeToAtom($a as any, () => {})); // Use functional API, add cast
        zenBatch(() => {
          for (const $a of atoms) {
            zenSetAtomValue($a, 1);
          }

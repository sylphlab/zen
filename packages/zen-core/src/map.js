...
import { listenKeys as addKeyListener, _emitKeyChanges } from './events'; // Import key listener logic AND _emitKeyChanges
            ...
            if (setLs?.size) { // Use size for Set
                for (const fn of setLs) { // Iterate Set
                    try {
                        ...
                        console.error(`Error in onSet listener for map key set ${String(mapAtom)}:`, e);
                    }
                }
            }
        }
                ...
                if (setLs?.size) {
                    for (const fn of setLs) {
                        try {
                            ...
                            console.error(`Error in onSet listener for map set ${String(mapAtom)}:`, e);
                        }
                    }
                }
            }

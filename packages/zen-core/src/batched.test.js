...
import { atom, set, batch } from './atom';
...
describe('batched', () => {
    ...
    test('calculates initial value after tick', async () => {
        ...
        const derived = batched(source, value => value * 2); // Cast source, add type hint for value
        const listener = vi.fn();
        expect(derived._value).toBeNull(); // Should be null initially
        const unsub = subscribe(derived, listener); // Cast derived
    ...
    test('batches multiple updates within one tick', async () => {
        ...
        const derived = batched([source1, source2], (v1, v2) => v1 + v2); // Cast sources, add type hints
        const listener = vi.fn();
        const unsub = subscribe(derived, listener); // Cast derived
    ...
    test('handles computed atom dependency initial null state', async () => {
        ...
        const calculationFn = (val) => {
            ...
            // Return something distinct if type is wrong, to help debugging
            console.error("!!! Unexpected type in batched calculation:", typeof val, val);
            return NaN; // Or throw error
        };
        const batchedComp = batched(comp, calculationFn); // Cast comp, add type hint
        const listener = vi.fn();
        // Subscribe to batchedComp, which should trigger subscription to comp
        const unsub = subscribe(batchedComp, listener); // Cast batchedComp
    ...
    test('does not notify if calculation result reference is the same', async () => {
        ...
        const unsub = subscribe(derived, listener);
    ...
    test('unsubscribes from source stores when last listener leaves', async () => {
        ...
        const derived = batched(source, calculation); // Cast source
        // expect(derived._unsubscribers).toHaveLength(0); // Remove check for internal property
        const unsub1 = subscribe(derived, () => { }); // Cast derived
        ...
        const unsub2 = subscribe(derived, () => { }); // Cast derived

...
describe('task', () => {
    ...
    const asyncFnError = vi.fn(async (arg) => {
        await tick();
    ...
    it('should notify subscribers on state changes', async () => {
        ...
        expect(listener).toHaveBeenLastCalledWith({ loading: true, error: undefined, data: undefined }, { loading: false, error: undefined, data: undefined });
        await promise;
        // Success state notification
        expect(listener).toHaveBeenCalledTimes(3);
        expect(listener).toHaveBeenLastCalledWith({ loading: false, error: undefined, data: 'Success: notify' }, { loading: true, error: undefined, data: undefined });
        unsubscribe();
    });

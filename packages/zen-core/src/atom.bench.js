...
const createJotaiWriteBenchSetup = (atomToWrite // Use imported WritableAtom type
) => {
    const store = createJotaiStore();
    const wrapper = ({ children }) => createElement(Provider, { store, children });
...
describe('Atom Set (No Listeners)', () => {
    ...
    const zustandStoreSet = createZustandVanillaStore((set) => ({
        count: 0,
    ...
    const effectorSetEvent = createEffectorEvent();

...
export function subscribe(atom, listener) {
        ...
        if (startLs?.size) {
            ...
            for (const fn of startLs) {
                try {
                    ...
                    console.error(`Error in onStart listener:`, e);
                }
            }
        }
    ...
    try {
        ...
        const initialValue = get(atom); // This handles computed updates
            ...
            if (stopLs?.size) {
                for (const fn of stopLs) {
                    try {
                        ...
                        console.error(`Error in onStop listener:`, e);
                    }
                }
            }
                ...
                for (const cleanupFn of cleanups.values()) {
                    if (typeof cleanupFn === 'function') {
                        try {
                            ...
                            console.error(`Error in onMount cleanup:`, e);
                        }
                    }
                }
...
export function queueAtomForBatch(atom, originalValue) {
    ...
    if (!batchQueue.has(atom)) { // Cast to unknown

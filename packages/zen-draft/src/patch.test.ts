...
interface SimpleState { a: number; b?: number | object }
interface NestedState { a: number; b: { c: number; d?: number }; list?: (number | object)[] }
interface ArrayState { list: (number | { id: number })[] }
interface MapState { map: Map<string, number | object> }
interface SetState { set: Set<number | string> }
   ...
   describe('"test" operation', () => {
     ...
     it('should throw if test fails (value mismatch)', () => {
      ...
      it('should throw if test fails (path not found)', () => {
       ...
       expect(() => applyPatches(baseState, patches)).toThrow(
         /'test' operation failed at path: b/,
       );
     });
   });

...
import { parseQuery, getPathFromUrl } from './utils';
...
describe('utils', () => {
  describe('parseQuery', () => {
    ...
    it('should parse multiple key-value pairs', () => {
      expect(parseQuery('?id=1&name=test&flag=true')).toEqual({ id: '1', name: 'test', flag: 'true' });
    });
    ...
    it('should handle encoded characters', () => {
      expect(parseQuery('?name=test%20user&data=%7B%22a%22%3A1%7D')).toEqual({ name: 'test user', data: '{"a":1}' });
    });
  ...
  describe('getPathFromUrl', () => {
    it('should return path, search, and hash from a full URL', () => {
      expect(getPathFromUrl('http://example.com/users/1?query=test#hash')).toBe('/users/1?query=test#hash');
    ...
    it('should return relative paths as is and warn (mock console.warn)', () => {
      ...
      expect(getPathFromUrl('users/1')).toBe('users/1');
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('[zen-router] Invalid URL or path provided'));
